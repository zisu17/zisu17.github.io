...
- **Pull (주기적)**: 모니터링 잡이 `GET .../tables/{table}` 로 스냅샷과 파일 통계를 주기적으로 폴링한다. 건강도 지표를 계산한다.
...
- **Push(엔진이 리포트 전송)와 Pull(카탈로그 폴링)** 두 축으로 테이블 건강도, 신선도, 파이프라인 정상성을 관측한다.
